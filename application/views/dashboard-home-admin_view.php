						...
						<div class="d-flex p5">
						  ...
						  <div class="mt-2 pt-3 pb-4 pl-4 pr-2 ml-2">
						  	...
						  	<p class="card-text pt-1"><small class="text-muted"><?php echo $post['account_username'];?> | Posted at <?php echo $post['post_dateCreated'];?></small></p>
							 	...
							 	<td><button onclick="update_event(<?php echo $post['post_id'];?>);" class ="btn mt-3 btn-success float-right f-normal c-radius mr-2">Edit</button></td>
						 <a href="<?php echo base_url('post/view_post/'.$post['post_slug']); ?>" class ="btn mt-3 btn-success float-right f-normal c-radius mr-2">Read More</a>
						  </div>
		...
		<div class="mb-3 col-11 card pl-0 pr-0 mx-auto shadow-light c-raduis pr-2">
		  <div class="card-body pt-5 p-4">
		   ...
		   <p class="card-text"><small class="text-muted"><em><?php echo $post['account_username'];?> | Posted at <?php echo $post['post_dateCreated'];?></em></small></p>
		    ...
		    <td><button onclick="update_announcement(<?php echo $post['post_id'];?>);" class ="btn mt-3 btn-success float-right f-normal c-radius mr-2">Edit</button></td>
		    <a href="<?php echo base_url('post/view_post/'.$post['post_slug']);?>" class ="btn mt-3 btn-success f-normal c-radius float-right mr-2">Read More</a>
		  </div>
		</div>
...
<div class="modal fade" id="editemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	<form method="post" action="<?php echo base_url('post/edit_event')?>">  
		<input class="form-control input-sm" id="inputsm" type="hidden" name="eventid">

		<div class="form-group">
 		 <label for="inputsm">Title</label>
		  <input class="form-control input-sm" id="inputsm" type="text" name="editetitle" placeholder="Title">
		</div>

		<div class="form-group">
		  <label for="inputsm">Event Date</label>
		  <input class="form-control input-sm" id="inputsm" type="date" name="editedate" placeholder="Event Date">
		</div>
		 				
		<div class="form-group">
		  <label for="comment">Description</label>
		  <textarea class="form-control" rows="5" id="comment" placeholder="Description" name="editecontent"></textarea>
		</div>
...
<div class="modal fade" id="editamodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	...
	<form method="post" action="<?php echo base_url('post/edit_announcement')?>">  
 		<input class="form-control input-sm" id="inputsm" type="hidden" name="announceid">

		<div class="form-group">
 		 <label for="inputsm">Title</label>
 		 <input class="form-control input-sm" id="inputsm" type="text" name="editatitle" placeholder="Title">
		</div>
 				
		<div class="form-group">
 		 <label for="comment">Description</label>
 		 <textarea class="form-control" rows="5" id="comment" placeholder="Content" name="editacontent"></textarea>
		</div>
